...
```
\abstract
This second homework covers the main concepts behind bayesian inference and some related issues. In the \textbf{first} and \textbf{second} exercises we focus on conjugate analysis, a classical approach to bayesian inference, used in the special occasion where the posterior distribution has the same distribution of the likelihood function. In the \textbf{third} and \textbf{fourth} exercise we introduce the Integral Transformation Formula, which will be the core, with unifrom pseudo-random numer generator of exercises \textbf{fifth} and \textbf{sixth} Finally in the \textbf{seventh}, \textbf{eighth} and \textbf{nineth} exerices we introduce and use the acceptance and rejection algorithm and apply in in a variety of situations.  
